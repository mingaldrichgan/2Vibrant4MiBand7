...
function renderBar(i, isMerged, currentKey, currentData, adjacentWidgetKey) {
  ...
  if (getProgress) {
    timer.createTimer(getProgress.ms, getProgress.ms, () => {
      if (level !== (level = getProgress.fn())) {
        arc.setProperty(hmUI.prop.LEVEL, getProgress.fn());
      }
    });
  }
  ...
  const textUI = hmUI.createWidget(hmUI.widget.TEXT_IMG, {
    _name: `bars[${i}].text`,
    x,
  ...
  if (getText) {
    timer.createTimer(getText.ms, getText.ms, () => {
      if (text !== (text = getText.fn())) {
        textUI.setProperty(hmUI.prop.TEXT, String(text));
      }
    });
  }
}
